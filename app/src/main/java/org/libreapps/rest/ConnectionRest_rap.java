...
public class ConnectionRest_rap extends AsyncTask<String, Void, String> {
    ...
    public String get(String methode) throws IOException, JSONException {
        ...
        InputStream is = null;
        String parameters = "";
        if(!methode.equals("POST")&&(jsonObj!=null)&&!methode.equals("CREATE_RAPPEL")){
        ...
        if (methode.equals("CREATE_RAPPEL")) {
            methode = "POST";
    ...
    public ArrayList<Rappel> parse(final String json) {
        try {
            final ArrayList<Rappel> list_rap = new ArrayList<>();
            final JSONArray jProductArray = new JSONArray(json);
            for (int i = 0; i < jProductArray.length(); i++) {
                list_rap.add(new Rappel(jProductArray.optJSONObject(i)));
            }
            return list_rap;
        } catch (JSONException e) {
